...
<!-- 账户管理弹框 -->
<el-dialog title="账户管理" :visible.sync="accountDialogVisible" width="600px">
  <el-form :model="accountInfo" label-width="100px">
    <el-form-item label="当前余额">
      <el-input v-model="accountInfo.balance" readonly :style="{ width: '200px' }"></el-input> <!-- 设置宽度 -->
    </el-form-item>
    ...
    <el-row :gutter="10">
      ...
      <el-col :span="12">
        <el-form-item label="操作">
          <el-button @click="confirmBalanceAdjustment(1)" type="primary" style="width: 100%;">增加</el-button>
          <el-button @click="confirmBalanceAdjustment(2)" type="danger" style="width: 100%;">减少</el-button>
        </el-form-item>
      </el-col>
    </el-row>
    ...
    <el-table :data="accountTransactions">
      ...
      <el-table-column label="变更类型" prop="transactionType">
        <template slot-scope="scope">
          {{ scope.row.isIncrease ? "添加":"扣除" }}
        </template>
      </el-table-column>
...
export default {
  ...
  handleAccountManagement(row) {
    ...
    this.getTransactionDetails(this.userId);  // 查询交易明细
  },
  ...
  confirmBalanceAdjustment(type) {
    ...
    this.$modal.confirm(`确认要${type === 1 ? '增加' : '减少'}编号 ${this.userId} 的余额 ${balanceAdjustment} 吗？`)
      .then(() => {
