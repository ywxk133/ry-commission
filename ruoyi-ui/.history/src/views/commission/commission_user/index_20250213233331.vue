...
export default {
    ...
    handleExport() {
     ...
     // 点击账户管理按钮后显示账户管理弹框
      handleAccountManagement(row) {
        console.log("handleAccountManagement",row.userId);
  ...
  const userId = this.userId;  // 获取当前用户ID

  // 使用箭头函数确保this的上下文
  this.$modal.confirm(`确认要${type === 1 ? '增加' : '减少'}编号 ${userId} 的余额 ${balanceAdjustment} 吗？`)
    .then(() => {
      // 用户确认操作后，执行增加或减少余额的操作
      if (type === 1) { // 增加余额
