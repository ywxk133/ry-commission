...
export default {
  ...
  methods: {
    ...
    // 确认余额调整
    confirmBalanceAdjustment(type,userId) {
      const userid = this.userid;
      console.log("this.userid ",JSON.stringify(this)+"userid",userid)
      ...
      this.$modal.confirm({
        title: '确认调整',
        content: `确认要${type === 1 ? '增加' : '减少'}编号 ${userid}  余额 ${balanceAdjustment} 吗？`,
        onOk: () => {
          if (type === 1) { // 增加余额
